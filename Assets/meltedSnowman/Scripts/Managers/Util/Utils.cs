...
namespace Utils
{
    ...
    {
        static public Transform GetRandomTransform(IEnumerable<Transform> list) {
        ...
        static public Vector3 GetRandomPoint(IEnumerable<Transform> list)
        {
            if (list.Count() == 0) return Vector3.zero;

            int ind = Random.Range(0, list.Count());
            return list.ElementAt(ind).position;
        }
